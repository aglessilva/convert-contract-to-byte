...
namespace Extrator_Tela_81
{
    public partial class Form1 : Form
    {
        ...
        public List<string> TrataArray(string _linha)
        {
            ...
            arrayLinha.RemoveAll(r => r.Equals("12,00") || r.Equals("25,00"));

            return arrayLinha;
        }


        private void NaoTem(string valor)
        {
            using (StreamWriter sw = new StreamWriter(@"C:\Tela81\Tela81_NAO_TEM.txt", true))
            {
                sw.WriteLine(valor);
            };
        }
                        ...
                        for (int i = 1; i <= reader.NumberOfPages; i++)
                        {
                            ...
                            using (StringReader strReader = new StringReader(pagina))
                            {
                                ...
                                while ((line = strReader.ReadLine()) != null)
                                {
                                    ...
                                    if (lista.Count <= 5)
                                    {
                                      //  NaoTem(string.Join(" ", lista.ToArray()) + " ARRAY MENOS QUE 8");
                                        continue;
                                    }

                                     _dataPagamento = lista[3];

                                    lista.RemoveRange(0, 7);
